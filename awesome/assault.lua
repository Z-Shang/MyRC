...
local acpi_is_on_ac_power = function (adapter)
	local f = io.open('/sys/class/power_supply/ADP0/online'):read()
...
local acpi_battery_percent = function (battery)
	local f = io.open('/sys/class/power_supply/BAT0/current_now')
	if f == nil then return 0 end
	local now = tonumber(f:read())
	local full = tonumber(io.open('/sys/class/power_supply/BAT0/charge_full'):read())
	return now / full
end
